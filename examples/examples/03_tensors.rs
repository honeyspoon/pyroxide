...
fn main() {
    ...
    let sum = unsafe { tensor_sum_f64(t.descriptor().as_raw()) };
    assert_eq!(sum, 15.0);
    ...
    let dot = unsafe { tensor_dot_f64(a.descriptor().as_raw(), b.descriptor().as_raw()) };
    assert_eq!(dot, 32.0);
    println!("  dot = {dot} [ok]");
    ...
    unsafe {
        tensor_matmul_f32(
            ma.descriptor().as_raw(),
            mb.descriptor().as_raw(),
            mc.descriptor().as_raw(),
        );
    };
