...
//   2. Call `.as_mojo().addr().as_raw()` to get a pointer as `isize`
//   3. Pass it to Mojo — Mojo reads fields at known byte offsets
//
// For mutation, use `.as_mojo_mut().addr().as_raw()` instead. Mojo writes
...
fn main() {
    ...
    let b = Point::ORIGIN;
    let dist = unsafe { point_distance(a.as_mojo().addr().as_raw(), b.as_mojo().addr().as_raw()) };
    ...
    let lum = unsafe { color_luminance(sky.as_mojo().addr().as_raw()) };
    ...
    unsafe { color_invert(c.as_mojo_mut().addr().as_raw()) };
